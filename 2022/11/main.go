...
// Monkeys sort interface implementation, to order by number of inspections
func (mks Monkeys) Len() int           { return len(mks) }
func (mks Monkeys) Less(i, j int) bool { return mks[i].inspections > mks[j].inspections }
func (mks Monkeys) Swap(i, j int)      { mks[i], mks[j] = mks[j], mks[i] }
...
// atoi interprets a string s and returns the corresponding worry level value
func atoi(s string) worryLevel {
	item, _ := strconv.Atoi(s)
	return worryLevel(item)
}
	...
	for _, input := range inputs {
		...
		case strings.HasPrefix(input, "Starting items:"):
			...
			for _, item := range strings.Split(strings.TrimPrefix(input, "Starting items: "), ", ") {
				monkey.items = append(monkey.items, atoi(item))
			}
		...
		case strings.HasPrefix(input, "Operation:"):
			...
			lhs, operator, rhs := atoi(args[0]), args[1], atoi(args[2])
...
func main() {
	...
	// 10605
	fmt.Println("part1:", monkeys.getBusiness())
	...
	// 2713310158
	fmt.Println("part2:", monkeys.getBusiness())
}
